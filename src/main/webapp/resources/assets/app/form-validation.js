  ...
  return {
    init: function () {
      ...
      jQuery.validator.addMethod('unique', function (value, element) {
        ...
        var url = form.attr('action').substring(0, form.attr('action').lastIndexOf('/') + 1) + 'checkUnique.json?element=' + $(element) .attr('name');
    ...
    initAjax: function ($container) {
      ...
      $('form.form-validation', $container) .each(function () {
        ...
        if (options.editrulesurl == undefined && $form.attr('method') .toUpperCase() == 'POST' && !$form.attr('formNotRule')) {
          var action = $form.attr('action');
          options.editrulesurl = action.substring(0, action.lastIndexOf('/')) + '/buildValidateRules.json'
        }
        ...
        $form.validate({
          ...
          submitHandler: function (form) {
            ...
            var dataLoadTarget = $form.attr('data-load-target');
            if (dataLoadTarget) {
              var $content = $(dataLoadTarget);
            ...
            $(form) .ajaxSubmit({
              ...
              success: function (response) {
                ...
                if (response.type == 'success' || response.type == 'warning') {
                  Global.notify(response.type, response.smessage);
                  ...
                  if (options.successCallback) {
                    ...
                    var toClose = submitButton.attr('data-to-close');
                    if (toClose) {
                      if (toClose == '_active-panel') {
                  ...
                  if (response.type == 'failure' || response.type == 'error') {
                    if (options.failureCallback) {
                      ...
                      Global.notify('error', response.smessage)
                    }
                  } else {
                      Global.notify('error', '保存失败，请重新填写')
...
//                  Global.notify('error', '表单处理异常，请联系管理员')
        ...
        $form.find('input[type=\'text\'],input[type=\'password\'],select,textarea') .each(function () {
          if (!$(this) .hasClass('form-control') && !$(this) .hasClass('calendar-select')) {
