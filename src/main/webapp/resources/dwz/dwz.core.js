...
var DWZ = {
	...
	ajaxDone:function(json,opt){
	    	var options = $.extend({},opt);
		if (json.statusCode === undefined && json.message === undefined) { // for iframeCallback
		    if (alertMsg) return alertMsg.error(json,options);
		    else return alert(json);
		} 
		if(json.statusCode == DWZ.statusCode.error) {
		    if(json.message && alertMsg) alertMsg.error(json.message,options);
		} else if (json.statusCode == DWZ.statusCode.timeout) {
		    if(alertMsg) alertMsg.error(json.message || DWZ.msg("sessionTimout"), $.extend({},opt,{okCall:DWZ.loadLogin}));
		    else DWZ.loadLogin();
		} else {
		    if(json.message && alertMsg) alertMsg.correct(json.message,options);
		};
	},
	getUnitBox :function(el){
	   var $el =  $(el);
	   var box =  $el.parents(".unitBox:first");
	   if(box.length==0){
    	   	var ctxm = $el.parents('#contextmenu');
    	   	if(ctxm.length!=0){
    	   	    var starget = ctxm.children().first().attr('target');
    	   	    if(starget){ box = $('#'+starget);}
    	   	}
	   } 
	   return box.length == 0 ? $(document) : box;
	},
...
(function($){
	...
	$.later = function(when, o, fn, data, periodic) {
	    ...
	    var result = {interval: periodic,cancel: function() {if (this.interval) {clearInterval(r);} else {clearTimeout(r);}}};
	    return result;
	};
	$.fn.extend({
		/**
		...
		layoutH: function($refBox){
			return this.each(function(){
				...
				if ($this.isTag("table")) {				    	
					$this.removeAttr("layoutH").wrap('<div class="scrollbar" layoutH="'+iLayoutH+'" style="overflow:auto;height:'+iH+'px"></div>');
					$this.parent().scrollbar();
				} else {
				    	var x = $this.attr('overflow-x') || 'auto';
				    	var y= $this.attr('overflow-y') || 'auto';
					$this.height(iH).css("overflow-x",x).css('overflow-y',y);
				}
			});
		},
