...
(function($){
	...
	function display(index, trigger, e, options) {
		var cur = hash[index];
		var obj = DWZ.frag[cur.id];
		if(!!!obj) {
		    var id = cur.id.substr(0,cur.id.length-2);
		    var el = $('[target*='+id+']');
		    el.trigger('beforeShow');
		    $("ul",el).first().attr("target",id);
		    obj = el.html();
		    el.on('click','li',function(){hide();});
		};
		var content = $(obj);
		if(content.text()==''){
		    return false;
		...
		if(menu.width()<20){menu.hide();if (cur.shadow){shadow.hide();}}
