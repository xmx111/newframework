			...
			$trs.hoverClass().each(function(){
				...
				$ftds.click(function(){
					...
					if($this.hasClass('enabledinput')){
					   ...
					   var active_value = $('input','.active-td').val();
					   $('.active-td').children().remove();
					   $('.active-td').text(active_value);
					   $('.enabledinput').removeClass('active-td');
					   var value = $this.text();
					   var el = $this;
					   el.html('');
					   el.addClass('active-td');
					   var html = [];
					   html.push('<input type="text" class="thisinput" name="" value="',value,'"/>');
					   el.append(html.join(''));
					   setTimeout("$('.thisinput').focus()",100);
					   initUI(el);
					}else{
					   var active_value = $('input','.active-td').val();
					   $('.active-td').children().remove();
