...
/**
 * navTabAjaxDone是DWZ框架中预订义的表单提交回调函数．
...
function navTabAjaxDone(json){
	...
	if (json.statusCode == DWZ.statusCode.ok){
		...
		} else if("refresh" == json.callbackType) {
		    refreshNavTab();
		}else {
...
function refreshNavTab(){
    var $pan = navTab.getCurrentPanel();
    var form = $('#pagerForm',$pan);
    if(form.length==0){
	navTab.reload();
    }else{
	navTabSearch(form);
...
function dialogAjaxDone(json){
	...
	if (json.statusCode == DWZ.statusCode.ok){
		if (json.navTabId){
		    ...
		    if($.pdialog._current){$.pdialog.closeCurrent();};
		} 
		if ("closeCurrent" == json.callbackType) {
...
function divSearch(form, rel){
	...
	if ($('input[name="pageIndex"]',$form).val()=='' && form[DWZ.pageInfo.pageNum]) form[DWZ.pageInfo.pageNum].value = 1;
	if (rel) {
		var $box = $("#" + rel);
		$box.ajaxUrl({
...
function _getPagerForm($parent, args) {
	var form = $("#pagerForm:first", $parent);
	if(form.length==0){ form = $("#pagerForm:first", $parent.parent());}
	form = form.get(0);
	if (form) {
...
function dwzPageBreak(options){
	...
	var $parent = op.targetType == "dialog" ? $.pdialog.getCurrent() : window.navTab ?  navTab.getCurrentPanel() : $(document);
...
function ajaxTodo(url, callback){
	var $callback = callback ? callback : window.navTab ? navTabAjaxDone : function(){};
