	...
	$.pdialog = {
		...
		open:function(url, dlgid, title, options) {
			...
			if(dialog) {
				...
				if(options.data) dialog.data("data",options.data);
...
//					$.pdialog.switchDialog(dialog);
			...
			if (op.mask) {
				...
				if(op.minable && $.taskBar && $.taskBar.addDialog) $.taskBar.addDialog(dlgid,title);
			}
		},
		/**
		 * 切换当前层
		...
		close:function(dialog) {
			...
			if($(dialog).data("mask")){
				...
				if ($(dialog).data("id") && $.taskBar && $.taskBar.closeDialog) $.taskBar.closeDialog($(dialog).data("id"));
			}
		...
		minimize:function(dialog){
			$(dialog).hide();
			$("div.shadow").hide();
			var task = $.taskBar &&  $.taskBar.getTask ?  $.taskBar.getTask($(dialog).data("id")) : null;
			$(".resizable").css({
				top: $(dialog).css("top"),
				left: $(dialog).css("left"),
				height:$(dialog).css("height"),
		...
		openDialogByID:function(id,op){
		    var options = {};
			var w = op.width?op.width:700;
			var h = op.height?op.height:500;
			if (w) options.width = w;
			if (h) options.height = h;
			options.max = op.max?op.max:false;
			options.mask = op.mask?op.mask:true;
			options.maxable = op.maxable?op.maxable:true;
			options.minable = op.minable?op.minable:true;
			options.fresh = op.fresh?op.fresh:true;
			options.resizable = op.resizable?op.resizable:true;
			options.drawable = op.drawable?op.drawable:true;
			options.close = op.close?op.close:true;
			options.param = op.param?op.param:{};
			this.open(id,"_blank",op.title, options);
	   }
	};
})(jQuery);
