...
public class CNCalendar {
    ...
    public static void main(String[] args) {
        CNCalendar l = new CNCalendar(System.currentTimeMillis());
    ...
    */
    private final static String[] sFtv = {
        ...
        "0910 教师节", "0928 孔子诞辰","1001*国庆节","1002*国庆节","1003*国庆节","1006 老人节",
    ...
    */
    private final static String[] lFtv = {
        "0101*春节","0102*春节","0103*春节","0106 定光佛诞", "0115 元宵节",
...
*/
    ...
    private synchronized void findFestival() {
        ...
        Matcher m;
        for (int i=0; i<CNCalendar.sFtv.length; i++) {
            m = CNCalendar.sFreg.matcher(CNCalendar.sFtv[i]);
            if (m.find()) {
                if (sM == CNCalendar.toInt(m.group(1)) && sD == CNCalendar.toInt(m.group(2))) {
        ...
        for (int i=0; i<CNCalendar.lFtv.length; i++) {
            m = CNCalendar.sFreg.matcher(CNCalendar.lFtv[i]);
            if (m.find()) {
                if (lM == CNCalendar.toInt(m.group(1)) && lD == CNCalendar.toInt(m.group(2))) {
        ...
        // 月周节日
        int w, d;
        for (int i=0; i<CNCalendar.wFtv.length; i++) {
            m = CNCalendar.wFreg.matcher(CNCalendar.wFtv[i]);
            if (m.find()) {
                if (this.getSolarMonth() == CNCalendar.toInt(m.group(1))) {
                    w = CNCalendar.toInt(m.group(2));
                    d = CNCalendar.toInt(m.group(3));
...
*/
    private static int getLunarLeapMonth(int lunarYear) {
        ...
        int leapMonth = CNCalendar.lunarInfo[lunarYear - 1900] & 0xf;
    ...
    */
    private static int getLunarLeapDays(int lunarYear) {
        ...
        // 若该年没有闰月,返回0
        return CNCalendar.getLunarLeapMonth(lunarYear) > 0 ? ((CNCalendar.lunarInfo[lunarYear - 1899] & 0xf) == 0xf ? 30
                : 29)
                : 0;
    }
    ...
    */
    private static int getLunarYearDays(int lunarYear) {
        ...
        for (int i = 0x8000; i > 0x8; i >>= 1) {
            daysInLunarYear += ((CNCalendar.lunarInfo[lunarYear - 1900] & i) != 0) ? 1
                    : 0;
        }
        // 加上闰月天数
        daysInLunarYear += CNCalendar.getLunarLeapDays(lunarYear);
    ...
    */
    private static int getLunarMonthDays(int lunarYear, int lunarMonth) {
        // 数据表中,每个农历年用16bit来表示,
        // 前12bit分别表示12个月份的大小月,最后4bit表示闰月
        int daysInLunarMonth = ((CNCalendar.lunarInfo[lunarYear - 1900] & (0x10000 >> lunarMonth)) != 0) ? 30
                : 29;
    ...
    */
    ...
    private static synchronized void makeUTCCalendar() {
        if (CNCalendar.utcCal == null) {
            CNCalendar.utcCal = new GregorianCalendar(TimeZone.getTimeZone("UTC"));
        }
    }
    /**
    ...
    */
    private static int getSolarTermDay(int solarYear, int index) {
        ...
        l = l + CNCalendar.UTC(1900,0,6,2,5,0);
        return CNCalendar.getUTCDay(new Date(l));
    }
    ...
    * @param TimeInMillis
    */
    public CNCalendar(long TimeInMillis) {
    ...
    private void init(long TimeInMillis) {
        ...
        this.lunarYear = 1900;
        int daysInLunarYear = CNCalendar.getLunarYearDays(this.lunarYear);
        while (this.lunarYear < 2100 && offset >= daysInLunarYear) {
            offset -= daysInLunarYear;
            daysInLunarYear = CNCalendar.getLunarYearDays(++this.lunarYear);
        }
        // 农历年数字
        ...
        int lunarMonth = 1;
        // 所在农历年闰哪个月,若没有返回0
        int leapMonth = CNCalendar.getLunarLeapMonth(this.lunarYear);
        // 是否闰年
        ...
        while (lunarMonth<13 && offset>0) {
            if (isLeap && leapDec) { // 如果是闰年,并且是闰月
                // 所在农历年闰月的天数
                daysInLunarMonth = CNCalendar.getLunarLeapDays(this.lunarYear);
                leapDec = false;
            } else {
                // 所在农历年指定月的天数
                daysInLunarMonth = CNCalendar.getLunarMonthDays(this.lunarYear, lunarMonth);
            }
    ...
    */
    private void getCyclicalData() {
        ...
        int term2 = CNCalendar.getSolarTermDay(solarYear, 2); // 立春日期
        // 依节气调整二月分的年柱, 以立春为界
        ...
        int firstNode = CNCalendar.getSolarTermDay(solarYear, solarMonth * 2); // 传回当月「节」为几日开始
        // 依节气月柱, 以「节」为界
        ...
        cyclicalDay = (int) (CNCalendar.UTC(solarYear, solarMonth, solarDay, 0, 0, 0) / 86400000 + 25567 + 10) % 60;
    ...
    */
    public String getAnimalString() {
        return CNCalendar.Animals[(this.lunarYear - 4) % 12];
    }
    ...
    */
    public String getTermString() {
        // 二十四节气
        String termString = "";
        if (CNCalendar.getSolarTermDay(solarYear, solarMonth * 2) == solarDay) {
            termString = CNCalendar.solarTerm[solarMonth * 2];
        } else if (CNCalendar.getSolarTermDay(solarYear, solarMonth * 2 + 1) == solarDay) {
            termString = CNCalendar.solarTerm[solarMonth * 2 + 1];
        }
        return termString;
    }
    ...
    */
    public int getTiananY() {
        return CNCalendar.getTianan(this.cyclicalYear);
    }
    ...
    */
    public int getTiananM() {
        return CNCalendar.getTianan(this.cyclicalMonth);
    }
    ...
    */
    public int getTiananD() {
        return CNCalendar.getTianan(this.cyclicalDay);
    }
    ...
    */
    public int getDeqiY() {
        return CNCalendar.getDeqi(this.cyclicalYear);
    }
    ...
    */
    public int getDeqiM() {
        return CNCalendar.getDeqi(this.cyclicalMonth);
    }
    ...
    */
    public int getDeqiD() {
        return CNCalendar.getDeqi(this.cyclicalDay);
    }
    ...
    */
    public String getCyclicaYear() {
        return CNCalendar.getCyclicalString(this.cyclicalYear);
    }
    ...
    */
    public String getCyclicaMonth() {
        return CNCalendar.getCyclicalString(this.cyclicalMonth);
    }
    ...
    */
    public String getCyclicaDay() {
        return CNCalendar.getCyclicalString(this.cyclicalDay);
    }
    ...
    */
    public String getLunarDayString() {
        return CNCalendar.getLunarDayString(this.lunarDay);
    }
    ...
    */
    public String getLunarMonthString() {
        return (this.isLeap() ? "闰" : "") + CNCalendar.getLunarMonthString(this.lunarMonth);
    }
    ...
    */
    public String getLunarYearString() {
        return CNCalendar.getLunarYearString(this.lunarYear);
    }
    ...
    */
    private static String getCyclicalString(int cyclicalNumber) {
        return CNCalendar.Tianan[CNCalendar.getTianan(cyclicalNumber)] + CNCalendar.Deqi[CNCalendar.getDeqi(cyclicalNumber)];
    }
    ...
    */
    private static String getLunarYearString(int lunarYear) {
        return CNCalendar.getCyclicalString(lunarYear - 1900 + 36);
    }
    ...
    */
    private static String getLunarMonthString(int lunarMonth) {
        ...
        if (lunarMonth == 1) {
            lunarMonthString = CNCalendar.lunarString2[4];
        } else {
            if (lunarMonth > 9)
                lunarMonthString += CNCalendar.lunarString2[1];
            if (lunarMonth % 10 > 0)
                lunarMonthString += CNCalendar.lunarString1[lunarMonth % 10];
        }
    ...
    */
    private static String getLunarDayString(int lunarDay) {
        ...
        int i1 = lunarDay / 10;
        int i2 = lunarDay % 10;
        String c1 = CNCalendar.lunarString2[i1];
        String c2 = CNCalendar.lunarString1[i2];
        if (lunarDay < 11) c1 = CNCalendar.lunarString2[0];
        if (i2 == 0) c2 = CNCalendar.lunarString2[1];
        return c1 + c2;
    }
}